...
def generate_gif(in_filename, out_filename, duracion):
    ...
    os.system(f'ffmpeg -ss {timess} -t 3 -i {in_filename} -filter_complex "subtitles={nombreSubtitulos(in_filename)}" -loop 0 {out_filename},fps=10,scale=480:-1:flags=lanczos,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse')
